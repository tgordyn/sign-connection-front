...
const VideoUploader = () => {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      console.log("request a:", <apiPath></apiPath>);
  ...
  const handleStartRecording = () => {
    startCountdown(3, () => {
      ...
      mediaRecorderInstance.onstop = () => {
        ...
        const file = new File([blob], `recorded-video-${uniqueId}.webm`, {
          type: "video/webm",
        });
        setVideoFile(file);
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex flex-col sm:flex-row mb-12" ref={tabsRef}>
        <button
          onClick={() => setActiveTab("upload")}
          className={`px-4 py-2 mr-0 mb-2 sm:mr-6 sm:mb-0 rounded ${
        ...
        <button
          ...
          className={`px-4 py-2 mr-0 mb-2 sm:mr-6 sm:mb-0 rounded ${
            activeTab === "record" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          Grabar Video
        </button>
      ...
      {activeTab === "upload" && (
        ...
        >
          <p className="text-gray-400 w-3/4 mt-12 mb-12 text-center text-xl">
            Arrastre un archivo de video en mp4 o{" "}
            <span
              onClick={handleTextClick}
              className="underline cursor-pointer"
            >
              haga click para seleccionar localmente
            </span>
          </p>
          <input
            type="file"
      ...
      {activeTab === "record" && (
        <div className="flex flex-col items-center mb-4 w-full max-w-xs sm:max-w-md h-auto">
      ...
      {activeTab === "view" && videoURL && (
        <div className="flex justify-center items-center mb-4 h-auto px-4">
          <video
            src={videoURL}
            controls
            className="mb-4 w-full max-w-[90%] sm:max-w-[640px] sm:h-[360px]"
            style={{ height: "auto" }}
          />
        </div>
      )}
      ...
      <div className="p-4 mt-4 border rounded bg-gray-100 w-3/4 text-center">
        <h2 className="text-xl font-bold mb-2">
          {error ? (
            ...
            <>
              Traducción:{" "}
              <span className="text-green-700">{videoFile.name}</span>
            </>
          ) : (
            ""
          )}
        ...
        <p className="text-lg">
          {error ? (
            ...
            videoFile && !translation && "Haga click en Traducir"
          )}
        </p>
